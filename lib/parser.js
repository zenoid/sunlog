...
exports.getLocationHistory = function ( startingDay, callback ) {
  ...
  var cookies = require( 'cookies.txt' ),
    ...
    cookiesFile = path.join( __dirname, '../googleCookies' ),
    ...
    locations = {},
    today = new Date(),
    endingDay = new Date( today.getFullYear(), today.getMonth(), today.getDate() );
...
exports.getWeatherData = function ( locations, firstDayNum ) {
  ...
  var apiKey,
    apiKeyFile = path.join( __dirname, '../forecastIOApiKey' ),
    apiURL = 'https://api.forecast.io/forecast/',
    apiParams = '?units=si&exclude=flags,hourly',
    weatherFile = path.join( __dirname, '../data/weather.json' ),
  ...
  var getWeatherDay = function( dayNum ) {
    ...
    request.get({
      ...
      weather[ day.getTime() ] = {
        ...
        temperatureMin: w.apparentTemperatureMin,
        temperatureMax: w.apparentTemperatureMax,
        cloudCover: w.cloudCover
      };

      console.log( day, w.icon, w.apparentTemperatureMin + '-' + w.apparentTemperatureMax );
